...
def health():
    # Этот эндпоинт для UptimeRobot и автопинга
    return "OK", 200

@app_web.route('/ping')
def ping():
    # Внутренний пинг для пробуждения
    return "pong", 200
# =======================================================
...
async def handle_audio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in pending_tracks:
        await update.message.reply_text("❌ Сначала используй /addfile, чтобы выбрать настроение для песни.")
        return
...
# ========== ФУНКЦИЯ ДЛЯ АВТОПИНГА (ЧТОБЫ НЕ ЗАСЫПАЛ) ==========
def keep_alive():
    """Каждые 14 минут пингует самого себя, чтобы Render не усыпил бота"""
